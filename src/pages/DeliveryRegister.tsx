...
import { Phone, User, Lock, Calendar } from "lucide-react";
...
const DeliveryRegister = () => {
  ...
  useEffect(() => {
    supabase.from("states").select("id, name").then(({ data }) => { if (data) setStates(data); });
  }, []);

  useEffect(() => {
    if (!stateId) { setDistricts([]); return; }
    supabase.from("districts").select("id, name").eq("state_id", stateId).then(({ data }) => { if (data) setDistricts(data); });
  }, [stateId]);

  useEffect(() => {
    if (!districtId) { setPanchayaths([]); return; }
    supabase.from("panchayaths").select("id, name").eq("district_id", districtId).then(({ data }) => { if (data) setPanchayaths(data); });
  }, [districtId]);

  useEffect(() => {
    if (!panchayathId) { setWards([]); return; }
    supabase.from("wards").select("id, ward_number").eq("panchayath_id", panchayathId).order("ward_number").then(({ data }) => { if (data) setWards(data); });
  }, [panchayathId]);

  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !mobile || mobile.length < 10 || !password || password.length < 6 || !panchayathId || !wardId || !dob) {
      toast({ title: "Missing fields", description: "Please fill all required fields. Password must be at least 6 characters.", variant: "destructive" });
      return;
    }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4 pt-24 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-lg">
        ...
        <form onSubmit={handleRegister} className="bg-card rounded-xl border border-border p-6 shadow-elevated space-y-4">
          <h2 className="font-display font-bold text-lg text-foreground text-center">Delivery Staff Sign Up</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={panchayathId} onValueChange={(v) => { setPanchayathId(v); setWardId(""); }} disabled={!districtId}>
                ...
                <SelectContent>{panchayaths.map((p) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              ...
              <Select value={wardId} onValueChange={setWardId} disabled={!panchayathId}>
                ...
                <SelectContent>{wards.map((w) => <SelectItem key={w.id} value={w.id}>Ward {w.ward_number}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
          ...
          <p className="text-center text-sm text-muted-foreground font-body">
            ...
            <Link to="/login/delivery" className="text-primary hover:underline font-medium">Login</Link>
          </p>
        </form>
      </motion.div>
    </div>
